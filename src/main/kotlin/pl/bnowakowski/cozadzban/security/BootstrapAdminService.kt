...
class BootstrapAdminService(
    ...
    @Value("\${COZADZBAN_BOOTSTRAP_ADMIN_EMAIL:}") private val bootstrapAdminEmail: String,
) {
    ...
    fun onApplicationReady() {
        ...
        if (email.isNullOrBlank()) {
            throw IllegalStateException(
                "No ADMIN users exist and COZADZBAN_BOOTSTRAP_ADMIN_EMAIL is missing or empty. " +
                    "Set this environment variable to the first admin's email."
            )
        }
        if (!looksLikeEmail(email)) {
            throw IllegalStateException(
                "COZADZBAN_BOOTSTRAP_ADMIN_EMAIL '$email' is not a valid email address."
            )
        }

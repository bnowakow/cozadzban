...
class FacebookProfileArticleImporterJobTest {
    ...
    fun `facebook marker phrase is not sent as quote when creating imported article`() {
        val importer = FacebookProfileArticleImporter(
            FacebookImportProperties(markerPhrase = "co za dzban", language = "pl"),
            appUserRepository,
            articleService,
        )
    ...
    fun `facebook marker phrase is not sent to remote article api for linked article`() {
        ...
        server.createContext("/api/articles") { exchange ->
            ...
            assertEquals("test-machine-key", exchange.requestHeaders.getFirst("X-CoZaDzban-M2M-Key"))
    ...
    fun `rejected candidate writes one debug artifact`(@TempDir tempDir: Path) {
        ...
        val method = importer.javaClass.getDeclaredMethod(
            ...
            Int::class.javaPrimitiveType,
            Class.forName(
                "pl.bnowakowski.cozadzban.facebookimport.FacebookProfileArticleImporter\$CandidateApprovalEntry",
            ),
        )
        method.isAccessible = true

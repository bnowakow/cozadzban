...
58a. **Spring Batch scheduled worker** — add Flyway-managed Spring Batch 6 JDBC metadata
    ...
    disable automatic Batch startup/schema initialization, and launch the job on
    the configured schedule when scheduling is enabled. Use Spring Batch
    metadata plus an in-process guard for v1 single-worker exclusivity.
59. **`FacebookImportController`** — expose admin-only explicit Selenium import endpoint
    `POST /api/admin/facebook-import/run/selenium`, keep `/run` as a Selenium
    compatibility alias, and expose `POST /api/admin/facebook-import/terminate`;
    return `202` on accepted start/terminate and `409` for busy/not-running.
60. **`ArticleListView` admin action** — add admin-only **Import Facebook Selenium**
    button next to "Add Article" when the Selenium importer is enabled.
...
61b. **Explicit Facebook Selenium import configuration** — split import configuration
    into shared orchestration and Selenium-only env vars. Spring Batch receives an
    explicit importType parameter and ADMIN UI exposes the Selenium action when
    available.
